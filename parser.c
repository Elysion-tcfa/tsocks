...
int make_netent(char *value, struct netent **ent) {
	...
	(*ent) -> af = af;
	if (((*ent) -> localip = malloc(getinaddrsize(af))) == NULL) {
		exit(1);
	}
	if (inet_pton(af, ip, (*ent) -> localip) != 1) {
		/* Badly constructed IP */
		free((*ent) -> localip);
		free(*ent);
		return(2);
	}
	subnet = strtol(strsplit(NULL, &split, " \n"), &badchar, 10);
	if (*badchar != 0 || subnet < 0 || subnet > 8 * getinaddrsize(af)) {
...
int __attribute__ ((visibility ("hidden"))) is_local(struct parsedfile *config, int af, void *testip) {
	...
	for (ent = config -> localnets; ent != NULL; ent = ent -> next)
		if (af == ent -> af && match(af, testip, ent -> localip, ent -> localnet))
			return 0;
