...
int send_request(int, void *server, void *req,
...
int main(int argc, char *argv[]) {
	...
	req[0] = '\x05';
	req[1] = '\x07';
	read_bytes = send_request(af, server, req,
...
int send_request(int af, void *server, void *req,
		 int reqlen, void *rep, int replen) {
	int sock;
	int rc;
	...
	if (connect(sock, (struct sockaddr *) server, getsockaddrsize(af)) == -1) {
