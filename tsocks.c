...
int connect(CONNECT_SIGNATURE) {
	...
	connaddr = (struct sockaddr *) __addr;

	/* Get the type of the socket */
	getsockopt(__fd, SOL_SOCKET, SO_TYPE,
			(void *) &sock_type, (socklen_t *) &sock_type_len);
	...
	if ((newconn = find_socks_request(__fd, 1))) {
		if (memcmp(newconn->connaddr, connaddr,
					getsockaddrsize(connaddr->sa_family))) {
	...
	if ((peer_address = (struct sockaddr *) malloc(namelen)) == NULL) {
		errno = ENOBUFS;
		return -1;
	}
	if (!getpeername(__fd, peer_address, (socklen_t *) &namelen)) {
		show_msg(MSGDEBUG, "Socket is already connected, defering to "
								 "real connect\n");
		return(realconnect(__fd, __addr, __len));
	}
		
	getipport(connaddr, &ip, &port);

	inet_ntop(connaddr->sa_family, ip, buf, 60);
	show_msg(MSGDEBUG, "Got connection request for socket %d to "
							 "%s\n", __fd, buf);

	/* If the address is local call realconnect */
	if (!(is_local(config, connaddr->sa_family, ip))) {
		show_msg(MSGDEBUG, "Connection for socket %d is local\n", __fd);
		return(realconnect(__fd, __addr, __len));
	}

	/* Ok, so its not local, we need a path to the net */
	pick_server(config, &path, connaddr->sa_family, ip, ntohs(port));
...
static struct connreq *new_socks_request(int sockid, struct sockaddr *connaddr,
	...
	if ((newconn->connaddr = (struct sockaddr *) malloc(connsize)) == NULL)
		return(NULL);
	memcpy(newconn->connaddr, connaddr, connsize);
	if ((newconn->serveraddr = (struct sockaddr *) malloc(serversize)) == NULL)
		return(NULL);
	memcpy(newconn->serveraddr, serveraddr, serversize);
...
static int connect_server(struct connreq *conn) {
	int rc;
	struct sockaddr *serveraddr = conn->serveraddr;
	void *ip;
	short port;
	char buf[60];

	/* Connect this socket to the socks server */
	getipport(serveraddr, &ip, &port);
	inet_ntop(serveraddr->sa_family, ip, buf, 60);
	show_msg(MSGDEBUG, "Connecting to %s port %d\n",
				buf, port);

	rc = realconnect(conn->sockid, (CONNECT_SOCKARG) serveraddr,
						  getsockaddrsize(serveraddr->sa_family));
...
static int send_socksv4_request(struct connreq *conn) {
	...
	thisreq->version = 4;
	thisreq->command = 1;
	thisreq->dstport = ((struct sockaddr_in *) connaddr)->sin_port;
	thisreq->dstip = ((struct sockaddr_in *) connaddr)->sin_addr.s_addr;
...
static int send_socksv5_connect(struct connreq *conn) {
	char constring[] = { 0x05,	 /* Version 5 SOCKS */
								...
								0x00,	 /* Reserved        */
								conn->connaddr->sa_family == AF_INET ? 0x01 : 0x04 };  /* IP Version 4    */
	...
	void *ip;
	short port;
	int size = getinaddrsize(conn->connaddr->sa_family);
	getipport(conn->connaddr, &ip, &port);
	memcpy(&conn->buffer[conn->datalen], ip, size);
	conn->datalen += size;
	memcpy(&conn->buffer[conn->datalen], &port, 2);
	conn->datalen += 2;
